...
directory = "differentiate/hypertuning/tuningWithRandomSearch"
project_name = "trials"
...
overwrite_checkpoint = True  # overwrite model saved

overwrite = True  # true = destroy previous results, false = resume search

config_tuner = {"n_epoch_max": 2000,
                "max_trials": 500,
...
with open(__file__+'/'+directory+'/config_tuner.json', 'w') as fp:
    json.dump(config_tuner, fp, indent=2)
#############################################################################
# Set F here
...
indices = np.random.randint(tf_coords.shape[0], size=5000)
...
class MyTuner(kt.RandomSearch):
    def __init__(self, hypermodel=None, objective=None, max_trials=10, seed=None, hyperparameters=None, tune_new_entries=True, allow_new_entries=True, validation_coords=None, **kwargs):
    ...
    def fit(self, hp, model, tf_coords, tf_boundary_coords, validation_coords=None, patience=10):
        ...
        history = {'train_loss': [],
                   'val_mae': []}
        n_epoch = config_tuner['n_epoch_max']
        for epoch in range(1, n_epoch+1):
            ...
            for _ in range(hp.Int('n_train', 10, 200)):
            ...
            print(f'train_loss: {mean_train_loss:.8f}', end=' ')
    ...
    def run_trial(self, trial, *args, **kwargs):
        ...
        history = self.fit(hp, model, tf_coords, tf_boundary_coords,
                           validation_coords=self.validation_coords)
...
if __name__ == '__main__':
    ...
    tuner = MyTuner(
        ...
        max_trials=config_tuner['max_trials'],
        executions_per_trial=config_tuner['executions_per_trial'],
        validation_coords=tf_val_coords,
        overwrite=overwrite,
        directory=directory,
