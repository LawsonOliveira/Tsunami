...
-laplacian(psi(x,y)) = f(x,y) on [0,1]^2  
With Dirichlet homogeneous boundary conditions psi(0,y)=psi(1,y)=psi(x,0)=psi(x,1)=0 
and f(x, y)=2*pi^2*sin(pi*x)*sin(pi*y)

The loss to minimize here is Loss = ||laplacian(psi(x,y)) + f(x,y)||_2
The true function psi should be psi(x, y)=sin(pi*x)sin(p*y)
...
class PINN:
    ...
    def target_function(self,inputs):
        return (2*jnp.pi**2*jnp.sin(jnp.pi*inputs[:,0])*jnp.sin(jnp.pi*inputs[:,1])).reshape(-1,1)
