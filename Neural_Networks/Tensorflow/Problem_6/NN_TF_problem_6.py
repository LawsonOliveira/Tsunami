...
a = 3  # appears in the functions f and true_function
...
@tf.function
def f(x):
    return tf.exp(-(a*x[:, 0]+x[:, 1])/5)*((-4/5*a**3*x[:, 0]-2/5+2*a**2)*tf.cos(a**2*x[:, 0]**2+x[:, 1])+(1/25-1-4*a**4*x[:, 0]**2+a**2/25)*tf.sin(a**2*x[:, 0]**2+x[:, 1]))
...
config = {
    ...
    'problem_id': 6,
    'remark': '',
    'over_folder_dir': 'Neural_Networks/Tensorflow/Problem_6/NN_saves/',
