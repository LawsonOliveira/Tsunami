...
def get_errors_L1_L2_poisson_dddd(nelemsx=10, nelemsy=10, verbosity=0):
    print("    --- get_errors_L1_L2_poisson_dddd ---")
    ...
    for i in range(nnodes):
        ...
        # set: u(x,y) = e^{ikx}
        solexact[i] = numpy.sin(PI*x)*numpy.sin(PI*y)
        laplacian_of_solexact[i] = -2*(PI**2)*solexact[i]
...
def get_alpha_error():
    ...
    for i, nelems in enumerate(nelems_values):
        ...
        solerr_normalized_L1, solerr_normalized_L2 = get_errors_L1_L2_poisson_dddd(
    ...
    matplotlib.pyplot.title(
        f"Study of MAE in function of h for Poisson equation:\n C={numpy.exp(lin_reg_res.intercept)}, alpha={lin_reg_res.slope}")
    matplotlib.pyplot.legend()
    dst_file_path = "./results/study_MAE_h_poisson"
    ...
    matplotlib.pyplot.plot(numpy.log(
        h_values), lin_reg_res.slope*numpy.log(h_values)+lin_reg_res.intercept, label='regression')
    matplotlib.pyplot.title(
        f"Study of MSE in function of h for Poisson equation:\n C={numpy.exp(lin_reg_res.intercept)}, alpha={lin_reg_res.slope}")
    matplotlib.pyplot.legend()
    dst_file_path = "./results/study_MSE_h_poisson"
    matplotlib.pyplot.savefig(dst_file_path)
    matplotlib.pyplot.show()
    matplotlib.pyplot.close()
...
if __name__ == '__main__':
    ...
    # print(get_errors_L1_L2_poisson_dddd(10, 10))
    get_alpha_error()
    print('End.')
