...
def update_boundary_v2(node_coords, p_elem2nodes, elem2nodes, boundary2nodes, n_subdivision):
...
def update_boundary(node_coords, p_elem2nodes, elem2nodes, boundary2nodes, global_ids, n_subdivision, local_boundary):
    # update boundary2nodes
    # local_boundary = np.array(
    #     [0, (n_subdivision+1)*n_subdivision, (n_subdivision+1)**2-1, n_subdivision, 0])
    l_loc_sequences_in_boundary = []
    for i in range(4):
        sequence_to_test = local_boundary[i:i+2]
        sequence_to_test = global_ids[sequence_to_test]  # get the global ids
        # print("sequence_to_test", sequence_to_test)
        index_boundary, is_in_bound = is_in_boundary(
            boundary2nodes, sequence_to_test)
        # print("is_in_bound", is_in_bound)
        if is_in_bound:
            print("the following segment is in boundary : ",
                  node_coords[sequence_to_test[0]], node_coords[sequence_to_test[1]])
            l_loc_sequences_in_boundary.append(
                (local_boundary[i:i+2], index_boundary))

    # print('l_loc_sequences_in_boundary', l_loc_sequences_in_boundary)

    n_nodes_added_to_boundary2nodes = 0
    for local_seq, index_boundary in l_loc_sequences_in_boundary:
        # print(local_seq)
        if np.abs(local_seq[1]-local_seq[0]) == n_subdivision:
            # horizontal edge
            if local_seq[1] > local_seq[0]:
                # upper edge
                nodes_to_add = global_ids[[local_seq[0] +
                                           i for i in range(1, n_subdivision)]]
            else:
                # print('lower edge')
                # lower edge
                nodes_to_add = global_ids[[
                    local_seq[0] - i for i in range(1, n_subdivision)]]
        else:
            # vertical edge
            if local_seq[1] > local_seq[0]:
                # left edge
                nodes_to_add = global_ids[[local_seq[0] +
                                           i*(n_subdivision+1) for i in range(1, n_subdivision)]]
            else:
                # right edge
                nodes_to_add = global_ids[[local_seq[0] -
                                           i*(n_subdivision+1) for i in range(1, n_subdivision)]]

        new_index_boundary = index_boundary+n_nodes_added_to_boundary2nodes
        boundary2nodes = np.concatenate(
            [boundary2nodes[:new_index_boundary+1], nodes_to_add, boundary2nodes[new_index_boundary+1:]], axis=0)
        n_nodes_added_to_boundary2nodes += nodes_to_add.shape[0]
...
def test_subdivide():
    ...
    node_coords, p_elem2nodes, elem2nodes, boundary2nodes = subdivide_all(
        node_coords, p_elem2nodes, elem2nodes, boundary2nodes, n_subdivision=10)
...
def subdivide_all(node_coords, p_elem2nodes, elem2nodes, boundary2nodes, n_subdivision=4):
    ...
    boundary2nodes = update_boundary_v2(
