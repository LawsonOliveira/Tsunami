...
def add_node_to_mesh(node_coords, p_elem2nodes, elem2nodes, new_node_coords, join_to_new_elem=True):
    ...
    if join_to_new_elem:
        id_2nearest_nodes = get_id_n_nearest_nodes(
        node_coords, new_node_coords)  # , is_inside_element, elem_included_in
        new_elem_node = np.concatenate(
            (id_2nearest_nodes, np.array([n_nodes])), axis=-1)
