...
from Models import architectures, pinns, operators, utils
...
config['checkpoint_path'] = '../Checkpoints/Third_approach/Helmholtz/'
...
config['img_path'] = '../Images/Third_approach/Helmholtz/'
...
####################################### Boundaries #########################################
############################################################################################
bound = jax.numpy.array([[0.0, 0.0], [0.0, 1.0], [1.0, 1.0], [1.0, 0.0]])
j_number = jax.lax.complex(0.0, 1.0)
bound_values = [lambda p: jax.numpy.exp(j_number*config['wave_number']*(0.0 + p[1])), 
                lambda p: jax.numpy.exp(j_number*config['wave_number']*(p[0]+1.0)), 
                lambda p:jax.numpy.exp(j_number*config['wave_number']*(1.0 + p[1])), 
                lambda p: jax.numpy.exp(j_number*config['wave_number']*(p[0] + 0.0))]
...
distance_class  = operators.Distance_functions(bound, bound_values, NN_eval)
solver = pinns.PINN_Helmholtz_third_approach(NN_eval, config['optimizer'], distance_class, config['wave_number'], config['reg'])
