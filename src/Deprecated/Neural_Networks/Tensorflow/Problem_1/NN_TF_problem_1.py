...
def f(x):
    return x**3+2*x+x**2*(1+3*x**2)/(1+x+x**2)


def residual(x, psi, dpsi_dx, f_ev):
    return dpsi_dx+(x+(1+3*x**2)/(1+x+x**3))*psi-f_ev
...
def try_config(config, id_add, use_wandb=False):
    ...
    def custom_loss(tf_sample_coords):
        ...
        F, dF_dx = evaluate_F_and_diff(tf_sample_coords)
        A, dA_dx = evaluate_A_and_diff(tf_sample_coords)

        psi_ev = A+tf.math.multiply(F, N)
        dpsi_dx = dA_dx+dF_dx*N + F*dN_dx
        f_ev = tf.reshape(f(tf_sample_coords), [
                          tf_sample_coords.shape[0], dim])

        res = residual(tf_sample_coords, psi_ev, dpsi_dx, f_ev)
    ...
    def validate(validation_coords):
        psi, dpsi_dx = differentiate(
            fun_psi, validation_coords, training=False)
        f_ev = tf.reshape(f(validation_coords), [
            tf.shape(validation_coords)[0], dim])
        res = residual(validation_coords, psi, dpsi_dx, f_ev)
...
def compare_truth(model_save_path):
    ...
    def true_function(X):
        return tf.exp(-X[:, 0]**2/2)/(1+X[:, 0]+X[:, 0]**3)+X[:, 0]**2
...
config = {
    ...
    'problem_id': 1,
    'remark': '',
    'over_folder_dir': 'Neural_Networks/Tensorflow/Problem_1/NN_saves/',
    "config_training": config_training,
    "config_model": config_model
}
