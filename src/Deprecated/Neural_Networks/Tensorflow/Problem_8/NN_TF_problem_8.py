...
dim = 2
# it is not sufficient to change these to change everything (expr_F and expr_A to tweak)
lb_0, lb_1 = 0, 0
ub_0, ub_1 = 1, 1
lower_bounds = [lb_0, lb_1]
upper_bounds = [ub_0, ub_1]
# mixed boundary conditions or not ? appears in the function expr_A()
mixed_bc_in_psi_x_1 = True
...
@tf.function
def f(x):
    return tf.sin(pi*x[:, 0])*(2-(pi*x[:, 1])**2+2*x[:, 1]**3*tf.sin(pi*x[:, 0]))


def residual(x, psi, dpsi_dx, dpsi_dxx, dpsi_dy, dpsi_dyy, f_ev):
    laplace_psi = dpsi_dxx+dpsi_dyy
    return laplace_psi+psi*dpsi_dy-f_ev


@tf.function
def differentiate(model, x, training=False, shift=None):
    '''
    Variable:
    -model: a NN model made with tensorlflow
    -x (array, shape=(None,2)): coordinates
    -training (bool): disable training or not
    -shift (dict): keys=['dim','degree'] to take the differentiate of model with respect to the given dimension shift['degree'] times
    returns:
    -u, du_dx, du_dxx, du_dy, du_dyy (u can be shifted)
    '''
    with tf.GradientTape(persistent=True) as tape:
        ...
        if shift is not None:
            wrt_dim = [x1, x2]
            for _ in range(shift["degree"]):
                u = tf.expand_dims(tf.squeeze(
                    tape.batch_jacobian(u, wrt_dim[shift["dim"]])), axis=-1)
...
def expr_A():
    ...
    def expr_g_1(x):
        return 2*sm.sin(np.pi*x)
    if mixed_bc_in_psi_x_1:
...
def try_config(config, id_add, use_wandb=False):
    ...
    def raw_residual(tf_sample_coords):
        x_1_coords = tf.concat(
            [tf_sample_coords[:, 0:1], tf.ones((tf_sample_coords.shape[0], 1), dtype=DTYPE)], axis=-1)
        N, dN_dx, dN_dxx, dN_dy, dN_dyy = differentiate(
            model, tf_sample_coords, training=True)

        N_x_1, dN_dx_x_1, dN_dxx_x_1, dN_dy_x_1, dN_dyy_x_1 = differentiate(
            model, x_1_coords, training=True)

        Ns_x_1, dNs_dx_x_1, dNs_dxx_x_1, dNs_dy_x_1, dNs_dyy_x_1 = differentiate(
            model, x_1_coords, training=True, shift={'dim': 1, 'degree': 1})  # Ns = dN_dy

        F, dF_dx, dF_dxx, dF_dy, dF_dyy = evaluate_F_and_diff(
            tf_sample_coords)
        A, _, dA_dxx, dA_dy, dA_dyy = evaluate_A_and_diff(
            tf_sample_coords)

        new_N = N-N_x_1-Ns_x_1
        new_dN_dx = dN_dx-dN_dx_x_1-dNs_dx_x_1
        new_dN_dxx = dN_dxx-dN_dxx_x_1-dNs_dxx_x_1
        new_dN_dy = dN_dy-dN_dy_x_1-dNs_dy_x_1
        new_dN_dyy = dN_dyy-dN_dyy_x_1-dNs_dyy_x_1

        psi = A+F*new_N
        # dpsi_dx = dA_dx+dF_dx*new_N+F*new_dN_dx
        dpsi_dxx = dA_dxx+dF_dxx*new_N+2*dF_dx*new_dN_dx+F*new_dN_dxx
        dpsi_dy = dA_dy+dF_dy*new_N+F*new_dN_dy
        dpsi_dyy = dA_dyy+dF_dyy*new_N+2*dF_dy*new_dN_dy+F*new_dN_dyy
...
config = {
    ...
    'problem_id': 8,
    'remark': '',
    'over_folder_dir': 'Neural_Networks/Tensorflow/Problem_8/NN_saves/',
